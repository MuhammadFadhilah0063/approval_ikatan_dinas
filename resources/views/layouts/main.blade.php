        ...
        <div class="sidebar-wrapper scrollbar scrollbar-inner">
          <div class="sidebar-content">
                ...
                <a data-toggle="collapse" aria-expanded="true">
                  <span>
                    ...
                    <span class="d-inline-block text-truncate"
                      style="max-width: 150px; font-size: 11.5px; font-weight: 700; color: black !important;">
                      {{ $user->nama }}
                    </span>
                    <span class="user-level" style="font-size: 11.5px; font-weight: 600;">
                      <!-- User Level -->
                      {{ $user->level }}
                    </span>
                  </span>
            ...
            <ul class="nav">
              ...
              <!-- Data Ikatan Dinas -->
              <li class="nav-item {{ Request::is('data-ikatan-dinas') ? 'active' : '' }}">
                <a class="collapsed" href="{{ route('ikatan_dinas') }}">
    ...
    <script>
      ...
      function setSelectPosisi(departemen) {
        ...
        if (departemen === 'PRODUKSI') {
          var posisi = ['PRODUCTION SECTION HEAD', 'OPERATOR DT 30 - 50', 'PRODUCTION GL', 'OPERATOR HD 785',
            'OPERATOR GD 825', 'PRODUCTION GL (ACT)', 'PRODUCTION SECTION HEAD (ACT)', 'PIT SERVICE GL', 'OPERATOR D155',
            'OPERATOR HD 465', 'OPERATOR BW 211D', 'MANPOWER GL', 'OPERATOR PC 2000', 'OPERATOR PC 1250',
            'HRM SECTION HEAD (ACT)', 'PRODUCTION DEPT HEAD', 'DRILL & BLAST GL', 'OPERATOR DT 20T', 'OPERATOR CAT 777',
            'OPERATOR WATER TRUCK', 'OPERATOR DT 20', 'OPERATOR D85', 'OPERATOR GD 705', 'OPERATOR PC 500',
            'OPERATOR PC 200', 'OPERATION INSTRUCTOR', 'DRILL & BLAST SECTION HEAD (ACT)', 'OPERATOR A2B'
          ];

        } else if (departemen === 'PLANT') {
          var posisi = ['MECHANIC WHEEL', 'PLANT WHEEL EQUIPMENT GL', 'PLANT FABRICATION GL', 'PLANT ADMIN',
            'MECHANIC TRACK', 'OPERATOR MANITOU', 'OPERATOR LUBE TRUCK', 'TYRE MAN', 'PLANT TRACK EQUIPMENT SECTION HEAD',
            'WELDER', 'PLANT SUPPORT EQUIPMENT GL', 'OPERATOR CT 50T', 'MECHANIC SUPPORT', 'PLANT DEPT HEAD',
            'PLANT TRACK EQUIPMENT GL', 'PLANT FABRICATION GL (ACT)', 'PLANT HEAVY EQUIPMENT GL', 'PLANT ENGINEER GL',
            'PLANT FABRICATION SECTION HEAD', 'PLANT PEOPLE DEVELOPMENT GL', 'PLANT TRACK EQUIPMENT GL (ACT)',
            'PLANT SUPPORT EQUIPMENT SECTION HEAD (ACT)', 'PLANT WHEEL EQUIPMENT GL (ACT)', 'OPERATOR WATER TRUCK',
            'OPERATOR LOWBOY', 'OPERATOR CT 10T', 'OPERATOR FORKLIFT', 'PLANT GL', 'PLANT PLANNER GL', 'PLANT TYRE GL',
            'PLANT SUPPORT EQUIPMENT GL (ACT)', 'PLANT SUPPORT EQUIPMENT SECTION HEAD', 'PLANT TYRE SECTION HEAD',
            'PLANT TRACK EQUIPMENT SECTION HEAD (ACT)', 'OPERATOR MERLO', 'PLANT ENGINEER GL (ACT)',
            'PLANT TYRE GL (ACT)', 'PLANT WHEEL EQUIPMENT SECTION HEAD', 'PLANT ENGINEER SECTION HEAD',
            'PLANT PEOPLE DEVELOPMENT SECTION HEAD', 'PLDP PLANT'
          ];

        } else if (departemen === 'HCGA') {
          var posisi = ['RECRUITMENT & DEVELOPMENT GL', 'GENERAL SERVICE GL', 'CHIEF SECURITY', 'SECURITY',
            'GENERAL AFFAIRS SECTION HEAD', 'COMBEN ADMIN', 'TRANSPORTATION GL', 'GENERAL SERVICE GL (ACT)',
            'GA ELECTRICIAN', 'CORPORATE SOCIAL RESPONSIBILITY GL', 'COMPENSATION & BENEFIT GL', 'INDUSTRIAL RELATION GL',
            'HUMAN CAPITAL SECTION HEAD', 'PLDP HCGA', 'HCGA DEPT HEAD', 'RECRUITMENT & DEVELOPMENT GL (ACT)'
          ];

        } else if (departemen === 'SHE') {
          var posisi = ['K3P SECTION HEAD', 'K3P GL', 'SHE DEPT HEAD', 'SHE ADMIN', 'EMERGENCY RESPONSE CREW', 'SHE HELPER',
            'KOP SECTION HEAD', 'KOP GL', 'EMERGENCY RESPONSE GL (ACT)', 'K3P SECTION HEAD (ACT)', 'PARAMEDIC',
            'EMERGENCY RESPONSE GL', 'SHE GL', 'ENVIRONMENT GL', 'DOCTOR', 'PLDP SHE'
          ];

        } else if (departemen === 'COE') {
          var posisi = ['ICT GL', 'OPERATION DISPATCH GL', 'CENTER OF EXCELLENCE SECTION HEAD', 'DATA PROCESSOR',
            'CENTER OF EXCELLENCE SECTION HEAD (ACT)', 'MANAGEMENT DEVELOPMENT GL', 'SYSTEM DEVELOPER', 'ICT GL (ACT)',
            'DISPATCH ANALYST GL', 'ICT TECHNICIAN', 'DISPATCH ANALYST GL (ACT)', 'PLDP COE'
          ];

        } else if (departemen === 'ENGINEERING') {
          var posisi = ['MINE PLAN SECTION HEAD', 'MINE PLAN GL', 'SURVEY GL', 'PIT CONTROL GL',
            'MINE PLAN SECTION HEAD (ACT)', 'ENGINEERING DEPT HEAD', 'SURVEY SECTION HEAD', 'SURVEY SECTION HEAD (ACT)',
            'MINEPLAN DRAFTER', 'SURVEY DRAFTER', 'CIVIL ENGINEER GL', 'DRILL & BLAST ENGINEER GL',
            'GEOTECHNOLOGY & DEVELOPMENT ENGINEER GL', 'PLDP ENGINEERING', 'SURVEY INSTRUMENT MAN'
          ];

        } else if (departemen === 'FA & LOGISTIK') {
          var posisi = ['FUEL ADMIN', 'FUEL GL', 'OPERATOR FUEL TRUCK', 'FALOG DEPT HEAD', 'FUEL MAN', 'LOGISTIC GL',
            'LOGISTIC ADMIN', 'LOGISTIC GL (ACT)', 'FUEL GL (ACT)', 'FINANCE & ACCOUNTING SECTION HEAD (ACT)',
            'PURCHASING ADMIN', 'LOGISTIC SECTION HEAD (ACT)', 'PURCHASING GL', 'FINANCE & ACCOUNTING GL',
            'PURCHASING GL (ACT)', 'PLDP FALOG', 'FINANCE & ACCOUNTING GL (ACT)'
          ];
        } else {
          var posisi = [];
        }
